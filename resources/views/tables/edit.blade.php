    ...
    <main class="flex-grow">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="max-w-md mx-auto bg-white rounded-lg shadow-md overflow-hidden">
                <div class="px-4 py-3">
                    <h2 class="text-xl font-light text-center text-soft-text mb-4">Modifier la table</h2>
                    <form action="{{ route('tables.update', $table->id) }}" method="POST">
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <input type="number" name="number" id="number" value="{{ $table->number }}" class="w-full px-2 py-1 text-sm border border-soft-secondary rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-soft-primary focus:border-soft-primary" required>
                            </div>
                            <div>
                                ...
                                <select name="location" id="location" class="w-full px-2 py-1 text-sm border border-soft-secondary rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-soft-primary focus:border-soft-primary" required>
                                    @foreach($locations as $value => $label)
                                        <option value="{{ $value }}" {{ $table->location === $value ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <label for="capacity" class="block text-xs font-medium text-soft-text mb-1">Capacité</label>
                                <input type="number" name="capacity" id="capacity" value="{{ $table->capacity }}" class="w-full px-2 py-1 text-sm border border-soft-secondary rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-soft-primary focus:border-soft-primary" required>
                            </div>
                            <div>
                                <label for="is_available" class="inline-flex items-center">
                                    <input type="checkbox" name="is_available" id="is_available" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="1" {{ $table->is_available ? 'checked' : '' }}>
